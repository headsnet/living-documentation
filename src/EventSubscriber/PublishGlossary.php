 ...
 */
final class PublishGlossary implements EventSubscriberInterface
{
    /**
     * @var Environment
     */
    ...
    public function build(PublishDocumentation $event)
    {
        try
        {
            $markdown = $this->twig->render('glossary.html.twig', [
                'data' => $event->getData(),
        ...
        $filesystem->dumpFile(
            sprintf('%s%s/glossary.md', $event->getOutDir(), $event->getContext()),
            $markdown
        );
    }
}
